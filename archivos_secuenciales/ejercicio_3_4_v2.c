...
void Actualizacion(int cantidad, float actualizacion, struct Producto precios[]){
    ...
    while(pos<cantidad){
        precios[pos].precio*=actualizacion;
        fwrite(&precios[pos], sizeof(struct Producto), 1, archivo);
        pos++;
    }
    ...
    while(pos<cantidad){
        fread(&precios[pos], sizeof(struct Producto), 1, archivo);
        printf("%d | %.2f | %s\n", precios[pos].codigo, precios[pos].precio, precios[pos].descripcion);
        pos++;
    }
